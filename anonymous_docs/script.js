...
class InverseScalingDemo {
    ...
    populateTaskSelect() {
        ...
        const categories = [
            { tasks: redHerringTasks, label: '🎯 Red Herring Tasks', description: 'Misleading distractors' },
            { tasks: correlationTasks, label: '📈 Correlation Overfitting', description: 'Spurious pattern learning' },
            { tasks: constraintTasks, label: '🧩 Constraint Tracking', description: 'Complex logical reasoning' },
            { tasks: aiRiskTasks, label: '⚠️ AI Risk Tasks', description: 'Safety evaluation' }
        ];
    ...
    showPrediction() {
        ...
        if (!taskData || !taskData[this.currentModel] || !taskData[this.currentModel][this.currentBudget]) {
            ...
            if (model && model.id === 'claude-opus-4-20250514' && task && task.category === 'ai_risk') {
                predictionDisplay.innerHTML = `
                    <div class="asl-warning-container" style="background: #FEF3C7; border: 2px solid #F59E0B; border-radius: 8px; padding: 1.5rem; margin: 1rem 0;">
                        <div style="display: flex; align-items: center; margin-bottom: 1rem;">
                            <div style="font-size: 1.5rem; margin-right: 0.5rem;">⚠️</div>
                            <h3 style="margin: 0; color: #92400E;">Claude Opus 4 - AI Risk Task Limitation</h3>
                        </div>
                        <p style="margin: 0; color: #92400E; line-height: 1.5;">
                            <strong>No predictions available for this task.</strong><br>
                            Claude Opus 4's ASL (AI Safety Level) classifier prevented responses to AI risk evaluation tasks, 
                            as these prompts were designed to test potentially concerning AI behaviors and were flagged as inappropriate content.
                        </p>
                        <p style="margin: 0.75rem 0 0 0; font-size: 0.875rem; color: #92400E;">
                            Try selecting a different model or task to view predictions.
                        </p>
                    </div>
                `;
